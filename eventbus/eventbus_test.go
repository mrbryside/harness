...
func TestEventBusQuestionAskedPayload(t *testing.T) {
	eb := NewEventBus()

	var result questionAskedData
	eb.Subscribe(EventQuestionAsked, func(e Event) {
		result = e.Data.(questionAskedData)
	})

	eb.Emit(EventQuestionAsked, questionAskedData{
		QuestionID: "startup-test",
		Question:   "Do you want to continue?",
	})

	if result.QuestionID != "startup-test" {
		t.Errorf("expected QuestionID='startup-test', got %q", result.QuestionID)
	}
	if result.Question != "Do you want to continue?" {
		t.Errorf("expected Question='Do you want to continue?', got %q", result.Question)
	}
}
