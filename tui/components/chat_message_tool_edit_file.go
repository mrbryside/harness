...
// toolEditContainerOverhead is the number of columns consumed by the diff
// container's left border (1) + left padding (2).
const toolEditContainerOverhead = 3

func (c *Chat) renderToolEditMessage(msg chatMessage) string {
	return c.RenderToolEdit(msg.toolEditPath, msg.toolEditOld, msg.toolEditNew, msg.toolEditStart) + messageGap
}

// toolEditChromaStyle is a custom chroma style that matches the app theme.
var toolEditChromaStyle = func() *chroma.Style {
...
// renderSimpleToolEdit is the fallback when the file can't be read or oldContent isn't found.
// startLine is the 1-based line number where oldContent starts in the file.
func (c *Chat) renderSimpleToolEdit(path string, oldContent, newContent string, startLine int) string {
	oldRaw := strings.Split(oldContent, "\n")
	newRaw := strings.Split(newContent, "\n")
	toolEditLines := computeRawToolEdit(oldRaw, newRaw, 3)
	...
	padding := lipgloss.NewStyle().
		Background(styles.Background).
		Width(c.width - toolEditContainerOverhead).
		Render("")
	...
	contentWidth := c.width - toolEditContainerOverhead - 7 - 2

	for _, dl := range toolEditLines {
		...
		case "context":
			...
			marker := lipgloss.NewStyle().
				...
				Width(1).
				Render(" ")
			wrapAndRenderLine(&sb, num, marker, highlighted, contentWidth, c.width-toolEditContainerOverhead, styles.Background)
		...
		case "remove":
			...
			marker := lipgloss.NewStyle().
				...
				Width(1).
				Render("-")
			wrapAndRenderLine(&sb, num, marker, highlighted, contentWidth, c.width-toolEditContainerOverhead, lipgloss.Color("#34232c"))
		...
		case "add":
			...
			marker := lipgloss.NewStyle().
				...
				Width(1).
				Render("+")
			wrapAndRenderLine(&sb, num, marker, highlighted, contentWidth, c.width-toolEditContainerOverhead, lipgloss.Color("#23303a"))
		}
	}

	return c.renderToolEditContainer(sb.String())
}
...
func highlightCode(code string, lang string) []string {
	...
	err = formatter.Format(&buf, toolEditChromaStyle, iterator)
	if err != nil {
		return strings.Split(code, "\n")
	}
...
// computeRawToolEdit computes a unified diff on raw strings with correct line numbers.
func computeRawToolEdit(oldLines, newLines []string, context int) []toolEditLine {
	...
	for oldIdx < len(oldLines) || newIdx < len(newLines) {
		if oldIdx < len(oldLines) && newIdx < len(newLines) && matches[oldIdx] == newIdx {
			...
			result = append(result, toolEditLine{
				kind:    "context",
...
func filterContext(diff []toolEditLine, context int) []toolEditLine {
	if len(diff) == 0 {
		return diff
	}
...
// RenderToolEdit renders a diff when the caller already knows the line numbers
...
func (c *Chat) RenderToolEdit(path, oldContent, newContent string, startLine int) string {
	...
	if err != nil {
		return c.renderSimpleToolEdit(path, oldContent, newContent, startLine)
	}
	defer file.Close()
	...
	toolEditLines := computeRawToolEdit(beforeLines, afterLines, 100)
...
// renderToolEditContainer wraps diff content in a styled container with a subtle left border.
func (c *Chat) renderToolEditContainer(content string) string {
